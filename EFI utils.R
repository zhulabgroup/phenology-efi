...
GPSDM<-function (pars, distMat,basisX, basisP, basisY=NULL, newX=NULL,newSigma=NULL,newP=NULL,newY=NULL,priors=NULL, priors_tr=NULL,Cinv.fixed=NULL,m.fixed=NULL,xnew=NULL, vnew=NULL, P_forecast=NULL,focalsites=NULL ,start=NULL, steps=NULL, lags=NULL, neighbors=NULL,vars=NULL,C_g=NULL,mode=c("basics", "optimize", "predict", "forecast","update")) {
  X<-basisX
  Y<-basisY
  P<-basisP
  ndim<-ncol(X)
  ...
  if (!is.null(Cinv.fixed)&!is.null(m.fixed) & mode!="optimize" ) { #must recalculate when optimizing
    ...
    #   }
    # }
    # kXX<-tau*exp(lC0)*exp(-Dist1^2*gamma)
  ...
  if (mode=="optimize") {
    ...
    if (!is.null(newY)) {
      ...
      dl[ndim+3]<-.5*vQ%*%matrix(dC[[ndim+3]], ncol=1)
      # dl[(ndim+4):(ndim+3+basisnumber*length(focalsites))]<-0.5*t(kXtX%*%Cinv)%*%Ctinv%*%(Yt-mt)+t(0.5*t(Yt-mt)%*%Ctinv%*%(kXtX%*%Cinv)) #is this correct?
    }
    
    # derivative for hyperparameters wrt transformed hyperparameters -- for gradient calculation
    dpars<-matrix(c((phi-phimin)*(1-(phi-phimin)/(phimax-phimin)),(ve-vemin)*(1-(ve-vemin)/(vemax-vemin)),(tau-taumin)*(1-(tau-taumin)/(taumax-taumin)),
                    (rho-rhomin)*(1-(rho-rhomin)/(rhomax-rhomin))
                    # (gamma-gammamin)*(1-(gamma-gammamin)/(gammamax-gammamin))
    ))
    ...
    if (is.null(priors)&is.null(priors_tr)) {
      ...
      if (!is.null(priors$a_ve)) {
        # tau
        lp_tau<-(priors$a_tau-1)*log(tau/taumax)+(priors$b_tau-1)*log(1-tau/taumax) #??? need jacobian?
        dlp_tau<-(priors$a_tau-1)/(tau/taumax)-(priors$b_tau-1)/(1-(tau/taumax))
        # process noise
        lp_ve<-(priors$a_ve-1)*log(ve/vemax)+(priors$b_ve-1)*log(1-ve/vemax)
        dlp_ve<-(priors$a_ve-1)/(ve/vemax)-(priors$b_ve-1)/(1-ve/vemax)
        # correlation
        lp_rho<-(priors$a_rho-1)*log(rho/rhomax)+(priors$b_rho-1)*log(1-rho/rhomax)
        dlp_rho<-(priors$a_rho-1)/(rho/rhomax)-(priors$b_rho-1)/(1-rho/rhomax)
        # lp_gamma<-(priors$a_gamma-1)*log(gamma/gammamax)+(priors$b_gamma-1)*log(1-gamma/gammamax)
        # dlp_gamma<-(priors$a_gamma-1)/(gamma/gammamax)-(priors$b_gamma-1)/(1-gamma/gammamax)
      ...
      if (!is.null(priors$E_ve)) {
        ...
        # lp_gamma<--.5*sum(((gamma-priors$E_gamma)^2)/priors$V_gamma)
        # dlp_gamma<--(gamma-priors$E_gamma)^1/priors$V_gamma
      }
