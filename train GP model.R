for (moy_oi in 1:12) {
  path_data<-paste0(path,"train/MOY", moy_oi)
  ...
  if (epoch == 1) {
    ...
    pars_prev_updated <- matrix(NA, nrow = ndim + 3, ncol = 0)
  ...
  ########################
  particles <- matrix(NA, nrow = ndim + 3, ncol = num_part)
  for (i in 1:ndim) {
  ...
  particles[ndim + 3, ] <- rtrunc(n = num_part, spec = "norm", a = gammamin, b = gammamax, mean = priors$E_gamma, sd = sqrt(priors$V_gamma))
  ...
  Rprop_res_add <-
    foreach(
      ...
      lpost <- function(p, sample_n, seed) {
        set.seed(seed)
        minibatch <- sample(1:nrow(X_train), sample_n)
        return(GPSDM(pars = p, distMat = distMat, basisX = X_basis, basisP = P_basis, basisY = Y_basis, newX = X_train[minibatch, , drop = F], newP = P_train[minibatch, , drop = F], newY = Y_train[minibatch, , drop = F], priors = priors, mode = c("optimize")))
      }
      print("start")
      
      fmingrad_Rprop(lpost, pars_add[, i, drop = F] + matrix(rnorm(ndim + 3, 0, 0.1)), sample_n = min(nrow(X_train),100), maxcount = 200)
    }
  
  pars_add_updated <- matrix(NA, nrow = ndim + 3, ncol = num_part)
  ...
  pars_var_add_updated <-
    foreach(
      ...
      num_sample <- 50
      pars_sample <- pars_add_updated[, i, drop = F] %*% matrix(1, nrow = 1, ncol = num_sample) + t(rmvn(num_sample, mu = rep(0, ndim + 3), Sigma = diag(1, nrow = ndim + 3, ncol = ndim + 3)))
      loglik_res <- rep(NA, num_sample)
